...
export async function main(ns: NS): Promise<void> {
  ...
  if (threads <= 0) {
    ...
    ns.tryWritePort(PREP_PORT, hostname);
    return;
  }

  ns.toast(`Prepping ${hostname}...`);
  while (true) {
    ...
    if (securityNotOK(ns, hostname)) {
      ns.run('/hacking/primitives/weak.js', threads, hostname);
      sleepTime += wTime;
    } else if (moneyNotOK(ns, hostname)) {
      ns.run('/hacking/primitives/grow.js', threads, hostname);
      sleepTime += gTime;
    } else {
      ns.run('/hacking/primitives/hack.js', threads, hostname);
      sleepTime += hTime;
    }

...
export async function main(ns: NS) {
    ...
    const handleServer = async (i: number) => {
        ...
        if (flags.check) {
            totalCost += cost;
            terminalOutput.push(`[CHECK] ${nameString(name)} ${currentRam ? ramString(currentRam) + ' -> ' : ''}${ramString(targetRam)}`);
            return;
        }
        ...
        terminalOutput.push(`${nameString(name)} ${currentRam ? ramString(currentRam) + ' -> ' : ''}${ramString(targetRam)}`);
    };

    if (flags.all) {
        for (let i = 0; i < maxServers; i++) await handleServer(i);
    } else {
        let index = purchased.length < maxServers
            ? purchased.length
            : parseInt(purchased.reduce((a, b) =>
